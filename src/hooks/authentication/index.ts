...
export const useAuthSignIn = () => {
    ...
    const onClerAuth = async (email: string, password: string) => {
        if (!isLoaded)
        ...
        try {
            ...
            if (authenticated.status === "complete") {
                ...
                toast("Success", { description: "Welcom back" })
    ...
    const { mutate: InitiateLogInflow, isPending } = useMutation({
        mutationFn: ({
            ...
            email: string
            password: string
        }) => onClerAuth(email, password),
    })

    const onAutheticatedUser = handleSubmit(async (value) => {
